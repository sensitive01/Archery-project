...
const Register = () => {
   ...
   const [formData, setFormData] = useState({
      ...
      declaration: false,
      aaiId: "", competitionsAttended: 0, competitionResults: "", studentPhoto: ""
   });
   ...
   const handlePhotoUpload = async (e) => {
      ...
      try {
         ...
         if (uploadedImage.secure_url) {
            setFormData((prev) => ({ ...prev, studentPhoto: uploadedImage.secure_url }));
   ...
   const handleNextStep2 = () => {
      if (formData.competitionsAttended === "" || formData.competitionsAttended === null) {
         showStatus("Please enter number of competitions attended", "error"); return;
      }
      if (Number(formData.competitionsAttended) > 0) {
         if (!formData.aaiId) {
            showStatus("AAI ID is mandatory if you have attended competitions", "error"); return;
         }
         if (!formData.competitionResults) {
            showStatus("Competition Results are mandatory if you have attended competitions", "error"); return;
         }
      }
      if (!formData.studentPhoto) {
         showStatus("Student photo is mandatory", "error"); return;
      }
      setCurrentStep(3);
   };
      ...
      <div style={{
         ...
         <div style={{
            ...
            <div style={{ display: 'flex', gap: '15px', marginBottom: '25px' }}>
               ...
               <div style={{ flex: 1, textAlign: 'center' }}>
                  <p style={{ fontSize: '10px', fontWeight: '900', color: currentStep === 3 ? '#1E40AF' : '#94a3b8', letterSpacing: '2px', marginBottom: '8px' }}>STEP 03</p>
                  <div style={{ height: '4px', backgroundColor: currentStep >= 3 ? '#1E40AF' : '#f1f5f9', borderRadius: '4px' }} />
               </div>
            </div>
            ...
            <div style={{ textAlign: 'center', marginBottom: '25px' }}>
               <div style={{ backgroundColor: '#eff6ff', width: '50px', height: '50px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 15px', color: '#1E40AF' }}>
                  ...
                  {currentStep === 2 && <Activity size={26} />}
                  {currentStep === 3 && <ShoppingCart size={26} />}
               </div>
               <h2 style={{ fontSize: '28px', fontWeight: '900', color: '#0F172A', marginBottom: '5px', letterSpacing: '-1px' }}>
                  {currentStep === 1 && "ATHLETE DETAILS"}
                  {currentStep === 2 && "SPORTS PROFILE"}
                  {currentStep === 3 && "CHECKOUT & SUMMARY"}
               </h2>
               ...
               <div style={{ animation: 'stepEnter 0.5s ease' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 200px), 1fr))', gap: '15px' }}>
                     ...
                     <button onClick={handleNextStep1} style={{ gridColumn: '1 / -1', backgroundColor: '#1E40AF', color: 'white', padding: '14px', borderRadius: '100px', border: 'none', fontWeight: '900', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', marginTop: '10px', transition: 'all 0.3s ease', boxShadow: '0 10px 30px rgba(30, 64, 175, 0.2)' }}>
                        CONTINUE TO SPORTS PROFILE <ChevronRight size={18} />
                     </button>
                  </div>
               </div>
            )}
               ...
               <div style={{ animation: 'stepEnter 0.5s ease' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 200px), 1fr))', gap: '15px' }}>
                     ...
                     <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', gridColumn: '1 / -1' }}>
                        <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', letterSpacing: '2px' }}>COMPETITIONS ATTENDED (NUMBER) *</label>
                        <input type="number" min="0" name="competitionsAttended" value={formData.competitionsAttended} onChange={handleChange} style={{ padding: '16px', borderRadius: '10px', border: '1px solid #f1f5f9', backgroundColor: '#f8fafc', outline: 'none', fontSize: '14px', fontWeight: '700' }} placeholder="0" />
                     </div>

                     <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', letterSpacing: '2px' }}>
                           AAI ID {Number(formData.competitionsAttended) > 0 ? "*" : "(OPTIONAL)"}
                        </label>
                        <input name="aaiId" value={formData.aaiId} onChange={handleChange} style={{ padding: '16px', borderRadius: '10px', border: '1px solid #f1f5f9', backgroundColor: '#f8fafc', outline: 'none', fontSize: '14px', fontWeight: '700' }} placeholder="Enter AAI ID" />
                     </div>
                     ...
                     <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        ...
                        <input 
                           ...
                           style={{ padding: '13px', borderRadius: '10px', border: '1px solid #f1f5f9', backgroundColor: '#f8fafc', outline: 'none', fontSize: '14px', fontWeight: '700', cursor: isUploadingPhoto ? 'not-allowed' : 'pointer' }} 
                        />
                        {isUploadingPhoto && <span style={{ fontSize: '12px', color: '#1E40AF', fontWeight: '600' }}>Uploading...</span>}
                        {formData.studentPhoto && !isUploadingPhoto && (
                           <span style={{ fontSize: '12px', color: '#16a34a', fontWeight: '600' }}>✓ Photo uploaded</span>
                        )}
                     </div>

                     <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', gridColumn: '1 / -1' }}>
                        <label style={{ fontSize: '10px', fontWeight: '900', color: '#94a3b8', letterSpacing: '2px' }}>
                           COMPETITION RESULTS {Number(formData.competitionsAttended) > 0 ? "*" : ""}
                        </label>
                        <textarea name="competitionResults" value={formData.competitionResults} onChange={handleChange} rows="3" style={{ padding: '16px', borderRadius: '10px', border: '1px solid #f1f5f9', backgroundColor: '#f8fafc', outline: 'none', fontSize: '14px', fontWeight: '700', resize: 'vertical' }} placeholder="Describe your competition results if any..." />
                     </div>

                     <div style={{ display: 'flex', gap: '15px', marginTop: '10px', gridColumn: '1 / -1' }}>
                        <button onClick={() => setCurrentStep(1)} style={{ backgroundColor: '#f1f5f9', color: '#64748B', padding: '14px 30px', borderRadius: '100px', border: 'none', fontWeight: '900', fontSize: '13px', cursor: 'pointer' }}>BACK</button>
                        <button onClick={handleNextStep2} style={{ flex: 1, backgroundColor: '#1E40AF', color: 'white', padding: '14px', borderRadius: '100px', border: 'none', fontWeight: '900', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', transition: 'all 0.3s ease', boxShadow: '0 10px 30px rgba(30, 64, 175, 0.2)' }}>
               ...
               <div style={{ animation: 'stepEnter 0.5s ease' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
                     ...
                     <div style={{ display: 'flex', gap: '15px', marginTop: '10px' }}>
                        <button onClick={() => setCurrentStep(2)} style={{ backgroundColor: '#f1f5f9', color: '#64748B', padding: '18px 30px', borderRadius: '100px', border: 'none', fontWeight: '900', fontSize: '13px', cursor: 'pointer' }}>BACK</button>
